...
import yaml
from condot.utils.helpers import parse_cli_opts
...
flags.DEFINE_string(
    "split", "", "Path to list of targets of particular split"
)
...
def name_expdir():
    ...
    if len(FLAGS.split) > 0:
        split_name = FLAGS.split.split('.')[0].split('-')[-1]
        if len(experiment_name) > 0:
            experiment_name = f"{experiment_name}/split-{split_name}"
        else:
...
def parse_config_cli(path, args):
    ...
    drug = FLAGS.drug
    split = FLAGS.split
    assert bool(drug) != bool(split) or not (bool(drug) and bool(split))
    if len(drug) > 0:
        config.data.target = drug
    if len(split) > 0:
        path_split = FLAGS.split
        config.data.target = list()
        with open(path_split, newline='') as f:
            for row in csv.reader(f):
                config.data.target.append(row[0])
