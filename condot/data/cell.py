...
# internal imports
from condot.models import load_autoencoder_model
from condot.utils import load_config
from condot.data.utils import cast_dataset_to_loader
from condot.utils.helpers import nest_dict
...
def read_single_anndata(config, path=None):
    ...
    # if not single target specified but all, set all remaining cells as target
...
def load_cell_data(
    ...
    **kwargs,
    ...
    if "ae_emb" in config.data:
        ...
        assert config.get("model.name", "condot") == "condot"
    ...
    if split_on is None:
        if config.model.name == "condot":
            if "property" in config.data and not isinstance(config.data.target, list):
                # datasets & dataloaders accessed as loader.train.source.property etc.
                split_on = ["split", "transport", config.data.property]
            else:
                # datasets & dataloaders accessed as loader.train.source.condition etc.
                split_on = ["split", "transport", config.data.condition]
    ...
    if len(split_on) > 0:
        splits = {
            ...
            for key, index in data.obs[split_on].astype('str').groupby(split_on).groups.items()
        }
