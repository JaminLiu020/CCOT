...
class Net2NetFlow_TFVAE_Covariate_Flow(nn.Module):
	...
	def train_cinn_hgraph(self, n_epochs = 400, batch_size = 128, lr = 4.5e-6, train_ratio = 0.8, seed = 42, start_epoch = 1,reverse = False, cov_concat = "Pert",auto_save = 0, auto_save_path = "./", perturb = False, num_workers = 0, pin_memory = False):
		"""
		train the net2net model, with train and test validation
		"""
		self.model_con.eval()
		...
		for epoch in tqdm(range(start_epoch, n_epochs + 1), desc="cINN train_cinn_hgraph", leave=True):
			train_loss, test_loss = 0, 0             
			self.flow.train()
			train_batches = tqdm(train_loader, desc=f"epoch {epoch}/{n_epochs} train", leave=False)
			for batch_idx, (batch_first_x, batch_cond_x, indices_cov_use) in enumerate(train_batches):
				if scvi_flag:                
					batch_first = torch.as_tensor(scvi_model.get_latent_representation(indices = indices_train[indices_cov_use], give_mean = False), dtype=torch.float32, device=device).unsqueeze(-1).unsqueeze(-1)
				else:                    
					batch_first = self.encode(batch_first_x).to(device)
				if not perturb:
					batch_cond_x = self.extractOneHot(batch_cond_x).to(device)                 
				else:
					batch_cond_x = torch.as_tensor(self.cond_stage_data[batch_cond_x], dtype=torch.float32, device=device)
				batch_cov_x = torch.as_tensor(self.covariates[indices_train[indices_cov_use]], dtype=torch.float32, device=device)
				if cov_concat == "Pert":
					batch_cond = torch.cat((batch_cond_x, batch_cov_x), 1)
				else:
					batch_first =torch.cat((batch_first.squeeze(-1).squeeze(-1), batch_cov_x), 1)
				...
				optimizer.step()
				train_loss += loss.item() * batch_first.shape[0]
				train_batches.set_postfix(loss=float(loss.item()))


			self.flow.eval()
			test_batches = tqdm(test_loader, desc=f"epoch {epoch}/{n_epochs} eval", leave=False)
			for batch_idx, (batch_first_x, batch_cond_x, indices_cov_use) in enumerate(test_batches):
				if scvi_flag:                
					batch_first = torch.as_tensor(scvi_model.get_latent_representation(indices = indices_test[indices_cov_use], give_mean = False), dtype=torch.float32, device=device).unsqueeze(-1).unsqueeze(-1)
				else:                    
					batch_first = self.encode(batch_first_x).to(device)
				if not perturb:
					batch_cond_x = self.extractOneHot(batch_cond_x).to(device)                 
					...
					batch_cond_x = torch.as_tensor(self.cond_stage_data[batch_cond_x], dtype=torch.float32, device=device)
				batch_cov_x = torch.as_tensor(self.covariates[indices_test[indices_cov_use]], dtype=torch.float32, device=device)
				if cov_concat == "Pert":
					batch_cond = torch.cat((batch_cond_x, batch_cov_x), 1)
# batch_first is actually always the coresponding to the cell states representations.
				...
				loss, _ = self.shared_step(batch_first, batch_cond, batch_idx, reverse = reverse)
				test_loss += loss.item() * batch_first.shape[0]
				test_batches.set_postfix(loss=float(loss.item()))
