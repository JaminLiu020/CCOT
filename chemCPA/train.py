...
def evaluate(
    ...
    """
    Measure quality metrics using `evaluate()` on the training, evaluate, and
    ...
    if run_disentangle:
        ...
        drug_names, drug_counts = np.unique(
            datasets["evaluate"].drugs_names, return_counts=True
        )
        disent_scores = evaluate_disentanglement(autoencoder, datasets["evaluate"])
        stats_disent_pert = disent_scores[0]
        # optimal score == always predicting the most common drug
        optimal_disent_score = max(drug_counts) / len(datasets["evaluate"])
    ...
    with torch.no_grad():
        evaluation_stats = {
            ...
            "optimal for covariates": [
                max(cov.mean(axis=0)).item() for cov in datasets["evaluate"].covariates
            ]  # mean over OHE embedding of covariates
            if datasets["evaluate"].num_covariates[0] > 0
            else None,
        }
        ...
        if run_r2:
            ...
            if "evaluate" in eval_stats:
                evaluation_stats["evaluate"] = eval_stats["evaluate"]
            else:
                evaluation_stats["evaluate"] = evaluate_r2(
                    autoencoder,
